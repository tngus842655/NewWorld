...
describe('군 프리셋 (2026-08-23 군 시스템)', () => {
  ...
  it('파견 — 카드 수 기반 동시 파견, 군 재파견 잠금', () => {
    ...
    const first = createExpedition(content, save, {
      regionId: 'misty-coast', tier: 'scout', partyIds: ['dune-pup'], artifactUids: [], teamId: 'team-1',
    }, clock.ctx);
    expect(first.expedition.teamId).toBe('team-1');
    expect(() => createExpedition(content, first.save, {
      regionId: 'misty-coast', tier: 'scout', partyIds: ['dune-pup'], artifactUids: [], teamId: 'team-2',
    ...
    expect(() => createExpedition(content, a.save, {
      regionId: 'misty-coast', tier: 'scout', partyIds: [], artifactUids: [], teamId: 'team-1',
